...
const AdminDashboard: React.FC<AdminPageProps> = ({ data, onUpdate }) => {
  ...
  const handleFinalSave = async () => {
    ...
    try {
      ...
      onUpdate(localData); // 부모 상태 업데이트 (성공 시)
      alert("✅ 모든 변경사항이 클라우드에 저장되었습니다.");
    } catch (err: any) {
  ...
  const saveCloudSettings = () => {
    ...
    alert("클라우드 설정이 저장되었습니다. 페이지를 새로고침합니다.");
  ...
  return (
    <div className="pt-32 pb-40 px-6 md:px-12 bg-black min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Sync Status Floating Bar */}
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[200] w-full max-w-md px-4">
          <div className="bg-zinc-900 border border-white/10 p-4 shadow-2xl flex items-center justify-between gap-4">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase">Status</span>
              <span className="text-[11px] text-white">
                {isSyncing ? '⏳ Saving to Cloud...' : syncError ? '❌ Error occurred' : '🟢 Ready to Save'}
              </span>
            </div>
            <button 
              onClick={handleFinalSave}
              disabled={isSyncing}
              className={`px-8 py-3 text-[10px] font-bold tracking-[0.3em] uppercase transition-all ${isSyncing ? 'bg-zinc-800 text-zinc-500' : 'bg-yellow-500 text-black hover:bg-yellow-400 active:scale-95'}`}
            >
              {isSyncing ? 'Wait...' : 'Save Changes'}
            </button>
          </div>
          {syncError && <p className="mt-2 text-red-500 text-[10px] text-center font-bold tracking-tight">{syncError}</p>}
        </div>
            ...
            <div className="grid md:grid-cols-3 gap-12">
              <div className="space-y-4">
                ...
                <div className="aspect-square bg-zinc-900 border border-white/10 relative group overflow-hidden">
                  <img src={localData.content.profileImage} className="w-full h-full object-cover grayscale transition-all group-hover:scale-110" />
              ...
              <div className="md:col-span-2 space-y-8">
                ...
                <div className="space-y-2">
                  ...
                  <textarea className="w-full bg-zinc-900 border border-white/5 p-4 text-sm outline-none focus:border-yellow-500 min-h-[150px] leading-relaxed" value={localData.content.aboutText} onChange={(e) => handleLocalUpdate({...localData, content: {...localData.content, aboutText: e.target.value}})} />
                </div>
              </div>
            </div>
          </div>
                ...
                <div key={p.id} className="grid md:grid-cols-4 gap-10 border-t border-white/5 pt-12 relative group">
                  ...
                  <div className="md:col-span-3 grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <select className="w-full bg-zinc-900 border border-white/5 p-3 text-xs outline-none focus:border-yellow-500 text-white" value={p.category} onChange={(e) => handleLocalUpdate({...localData, projects: localData.projects.map(x => x.id === p.id ? {...x, category: e.target.value as Category} : x)})}>
                    ...
                    <div className="col-span-2 space-y-2">
                      ...
                      <textarea className="w-full bg-zinc-900 border border-white/5 p-3 text-sm outline-none focus:border-yellow-500 min-h-[80px]" value={p.synopsis} onChange={(e) => handleLocalUpdate({...localData, projects: localData.projects.map(x => x.id === p.id ? {...x, synopsis: e.target.value} : x)})} />
                    </div>
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'CLOUD' && (
          <div className="max-w-2xl space-y-12 animate-fade-in mb-20">
            <div className="p-8 bg-zinc-900/50 border border-yellow-500/10 space-y-6">
              <h3 className="text-sm tracking-[0.3em] font-bold text-yellow-500 uppercase">Step 1: Get Keys</h3>
              <p className="text-zinc-400 text-sm leading-relaxed">
                <a href="https://jsonbin.io" target="_blank" className="text-yellow-500 underline">JSONbin.io</a> 접속 -> API Keys 탭 -> Master Key 복사. <br/>
                Bins 탭 -> Create Bin -> 생성된 Bin ID 복사.
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <button onClick={saveCloudSettings} className="w-full py-4 bg-yellow-500 text-black font-bold uppercase text-[10px] tracking-[0.4em]">Connect & Restart</button>
            </div>
          </div>
        )}
      </div>
